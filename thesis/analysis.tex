...
\end{equation} 
We investigate the frequencies of all considered categories, namely queries, named entities, words and characters, and evaluate if they resemble Zipf's law. To achieve this, we sort the frequencies of linguistic elements in descending order and display them in a log-scaled graph. Albeit primarily studied for words, we attempt to retrieve Zipf's law also in the frequencies of queries, named entities and characters since they as well are linguistic categories and probably follow linguistic dynamics. 
\subsubsection{Visual Comparison: Named Entities and Words}
Figure~\ref{fig:zipf-named-entities-words} shows the ordered frequencies of named entities and words in the query logs. As for named entities, we can state that all distributions follow Zipf's law reasonably well. The distributions show a relatively constant slope in the log-scaled dimensions and are fairly similar. This is mostly also true for the word frequencies, despite a small deviation: The constant slope is interrupted by a small curvature in the central part of the distribution. However, this curvature is present in all distributions. From a visual perspective, we can conclude that the distributions of the different data sets are quite similar. There are no striking outliers which indicate clear differences.

 
\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\textwidth]{../plots/updated_plots/extract-named-entities-and-extract-words-single/all.pdf} 
  \caption{The relative frequencies of extracted named entities and words are displayed in a log-scaled graph for each query log. The frequencies were ordered in a descending order to create a ranking. Hence the depiction of frequencies and the rank. A good fit to Zipf's law is indicated by a straight line in the log-scaled dimensions.} % The frequencies were ordered in a descending order to create a ranking. Hence the depiction of frequencies and the rank. A good fit to Zipf's law is indicated by a straight line in the log-scaled dimensions.
...
The distributions of linguistic elements are now compared numerically to each other. We apply the \textit{Kolomogerov-Smirnov-Statistic} (KS-Statistic) to evaluate similarities between the resulting distributions. Considering two empirical measures $X$ and $Y$ with samples $x_i$ and $y_i$, the KS-Statistic is defined as:
\begin{equation}
    \mathrm{D_{KS}(X,Y)}=\sup _{x}\left|F_{X}(x)-F_{Y}(x)\right|
\end{equation}
where $F_{X}$ and $F_{Y}$ are the empirical cumulative distribution functions of $X$ and $Y$. The values of the KS-Statistic range from 0 to 1, where 0 indicates that the distributions are identical and 1 indicates that the distributions are completely different. To make a meaningful comparison, we are primarily interested in the relative differences of the KS-Statistic and not in its absolute values. Therefore, we compute pairwise comparisons of the KS-Statistic for each query log and each category. In Table~\ref{tab:ks-statistic} the KS-Statistic of each query log is displayed with regard to the conducted analysis. The values are not significantly different, hence we can state that the AQL fits the other logs in structural or linguisitc terms reasonably well. In Figure~\ref{fig:ks-statistic} a configuration of the distributions is displayed that visualizes the KS-Statistic. Each point corresponds to a query log's distribution and the configuration of the points reflects the KS-Statistic to each other. The configuration was derived by applying multidimensional scaling to the resulting distance matrix. As the numbers already indicated, the points are distributed quite evenly and no clear clusters are visible.
\begin{table}
    \centering
    \scalebox{0.9}{
    \begin{tabular}{@{}lrrrr@{}} \toprule
        ...
        AOL & - & 0.18 & 0.25 & 0.07  \\ 
        AQL & 0.18 & - & 0.39 & 0.20    \\
        MS WS & 0.25 & 0.39 & - & 0.20   \\
        ORCAS & 0.07 & 0.20 & 0.20 & -  \\ \midrule
        &  \multicolumn{4}{c}{\textbf{Words}} \\ \cmidrule(l{2pt}r{2pt}){2-5} 
        & AOL & AQL & MS WS & ORCAS  \\ 
        AOL & - & 0.64 & 0.20 & 0.26   \\ 
        AQL & 0.64 & - & 0.46 & 0.42    \\
        MS WS & 0.20 & 0.46 & - & 0.16   \\
        ORCAS & 0.26 & 0.42 & 0.16 & -  \\ \midrule
        &  \multicolumn{4}{c}{\textbf{Characters}} \\ \cmidrule(l{2pt}r{2pt}){2-5}
        & AOL & AQL & MS WS & ORCAS  \\ 
        AOL & - & 0.52 & 0.79 & 0.11   \\ 
        AQL & 0.52 & - & 0.34 & 0.49    \\
        MS WS & 0.79 & 0.34 & - & 0.74   \\
        ORCAS & 0.11 & 0.49 & 0.74 & -  \\ 
        \bottomrule
    \end{tabular}
    }
    \caption{The KS-Statistic of each query log is displayed with regard to the respective linguistic category. The values are not significantly different, hence we can state that the AQL fits the other logs in structural or linguisitc terms reasonably well.}
    \label{tab:ks-statistic}
\end{table}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{../plots/KS-Distances-Linguistic-Elements/all.pdf} 
  \caption{The configuration of the KS-Statistic is displayed. Each point corresponds to a query log's distribution and the configuration of the points reflects the KS-Statistic to each other. The configuration was derived by applying multidimensional scaling to the resulting distance matrix.}
...
\begin{table}[h]
    ...
    \begin{tabular}{@{}rlrrlrrlrr@{}} \toprule
        & \multicolumn{3}{c}{\textbf{AQL}} & \multicolumn{3}{c}{\textbf{MS-MARCO}} & \multicolumn{3}{c}{\textbf{ORCAS}} \\ \cmidrule(l{2pt}r{2pt}){2-4} \cmidrule(l{2pt}r{2pt}){5-7} \cmidrule(l{2pt}r{2pt}){8-10} 
...
In Figure~\ref{fig:query-intent} the distribution of query intents in the query logs is displayed. The labels were produced by the classifier of \citet{alexander:2022}. The figure shows an equal ranking of the labels. The most frequent label is ``informational'' followed by ``navigational'' and ``transactional''. Apart from the ratio of navigational queries, the distributions are very similar.  
\begin{figure}[t!]
    \centering
...
\subsection{TBD Question Detection}
In Figure~\ref{fig:question-detection} the ratio of questions in the query logs is displayed. The labels were produced by a rule-based classifier provided by \citet{reimer:2023a}. The figure shows that the ratio of questions is very low in all query logs. 
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{../plots/questions/all.pdf} 
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\textwidth]{../plots/Wasserstein-Distances-Lengths/all.pdf} 
