...
def get_input_paths(dataset_name):
    if dataset_name == 'aql':
        ...
        return input_paths_aql
    elif dataset_name == 'aol':
        return '/mnt/ceph/storage/data-in-progress/data-teaching/theses/thesis-schneg/corpus-aol-cleaned/'
    elif dataset_name == 'ms-marco':
        return '/mnt/ceph/storage/data-in-progress/data-teaching/theses/thesis-schneg/corpus-msmarco/'
    elif dataset_name == 'orcas':
        return '/mnt/ceph/storage/data-in-progress/data-teaching/theses/thesis-schneg/orcas_cleaned/'
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Please provide the dataset name as an argument. Use \"aql\", \"aol\", \"ms-marco\" or \"orcas\" as dataset name.")
        sys.exit(1)

    # Get the dataset name from command line arguments
    dataset_name = sys.argv[1]
    assert dataset_name in ['aql', 'aol', 'ms-marco',
                            'orcas'], "Dataset name not supported! Use \"aql\", \"aol\", \"ms-marco\" or \"orcas\" as dataset name."

    input_paths = get_input_paths(dataset_name)

    if dataset_name == 'aol':
        aol_parse_options = csv.ParseOptions(delimiter="\t")
        aol_dataloader = Ray_Dataloader(
            file_type="txt", path_dataset=input_paths, compression='gz', parse_options=aol_parse_options, includePath=True)
        ...
        ds_aol = ds_aol.map_batches(
            fill_missing_columns, batch_format="pandas")
        ...
        ms_dataloader = Ray_Dataloader(
            file_type="tsv", path_dataset=input_paths, parse_options=ms_parse_options)
        ...
        orcas_dataloader = Ray_Dataloader(
            file_type="csv", path_dataset=input_paths)
        ds_orcas = orcas_dataloader.read_file()
        ds_orcas = ds_orcas.map_batches(
            fill_missing_columns, batch_format="pandas")
